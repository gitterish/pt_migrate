...
#Project
	...
	if ($pt_labels =~ /ee[,<]/)
	{
		$ot_item->{project}->{id} = $OPTIONS->{OT_PROJECT_ENTERPRISE};#Convirt-Enterprise
	}
	elsif ($pt_labels =~ /ec2,/ or $pt_labels =~ /ec2$/)
	{
		$ot_item->{project}->{id} = $OPTIONS->{OT_PROJECT_CLOUD};#Convirt-Cloud
	}
	else
	{
#Default
		$ot_item->{project}->{id} = $OPTIONS->{OT_PROJECT};
	}
...
sub process_story
{
	...
	if ($OPTIONS->{SkipAccepted} && $ot_input->{item}->{workflow_step}->{id} == $WORKFLOW_STEP_COMPLETED)
	{
...
{
	print $error . "\n";
	print "Usage: $0 --cfgfile=<cfg file> --storyfile=<story file> --mapfile=<map file> --post\n";
	exit;
}
